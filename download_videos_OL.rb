...
def still_downloading?
  latest_file_ext = latest_entry(@home_dir)
  latest_file_ext = latest_download_ext.split('.').last unless latest_download_ext.nil?
...
def attempt_login
  ...
  url = "https://www.dailydrip.com/users/sign_in"
...
def process link, index
  ...
  if no_download?
    ...
    path = '/Users/kned/Downloads/DailyDrip/scraping'
    # newest_file = Dir.glob("#{path}/*").max_by {|f| File.mtime(f)}
    newest_file = Dir.glob("#{@home_dir}/*").max_by {|f| File.mtime(f)}
    newest_file = [File.basename(newest_file)]
  else
    download_or_next
...
def url_by_topic page_no
  ...
  topic_id = 16
  "https://www.dailydrip.com/drips/search?availability=all&order=oldest&page=#{page_no}&q=&topic_id=#{topic_id}&user_id=6197&utf8=%E2%9C%93&watched=all"
end
...
def download_vids
  1.upto(2) do |page_no|
    ...
    links.each_with_index do |link, i|
      ...
      # if index < @skip_to && page_no == @skip_to_page
...
error_msg = "Unable to login. Please check credentials"

successful_login = attempt_login
sleep 5
successful_login ? download_vids : (puts error_msg)
